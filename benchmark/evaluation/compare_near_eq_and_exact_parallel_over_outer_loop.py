...
testsystems = {#"alanine_constrained": alanine_constrained,
...
dt_range = np.arange(5.0,8.01,0.5) * unit.femtosecond # just restricting to the dt's that haven't completed yet...
...
def collect_inner_samples_until_threshold(x, v, noneq_sim, marginal="full", initial_size=100, batch_size=5,
    ...
    while (stdev_log_rho_pi(np.array(Ws)) > threshold) and (len(Ws) <= (max_samples - batch_size)):
        print("{} samples so far, but stdev_log_rho_pi ({:.3f}) > threshold ({:.3f}), collecting another batch!".format(len(Ws), stdev_log_rho_pi(np.array(Ws)), threshold))
